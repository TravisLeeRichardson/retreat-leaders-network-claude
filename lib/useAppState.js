...
export function useAppState() {
  ...
  const v = {
    isGuest: !s.user,
    isAuthed: !!s.user,
    userLabel: s.user === "organizer" ? "Travis" : "Dr. Okafor",
    toast: s.toast,
    ...
    navLogin: () => go("login"),
    navDash: () => {
      if (!s.user) patch({ user: "organizer" });
      go("dash");
    },
    navApps: () => go("apps"),
    navProfile: () => {
      patch({ leaderId: s.user === "organizer" ? "travis" : "maya" });
      go("leader");
    },
    ...
    leaderIsApplicant: leader.applicant && s.user === "organizer",
    ...
    roleLabel: s.loginRole === "leader" ? "Leader" : "Organizer",
    leaderRoleBd: s.loginRole === "leader" ? A : "#e8ded2",
    leaderRoleBg: s.loginRole === "leader" ? "#f9f0e8" : "#fffdfa",
    orgRoleBd: s.loginRole === "organizer" ? A : "#e8ded2",
    orgRoleBg: s.loginRole === "organizer" ? "#f9f0e8" : "#fffdfa",
    pickLeader: () => patch({ loginRole: "leader" }),
    pickOrganizer: () => patch({ loginRole: "organizer" }),
    doLogin: () => {
      const role = s.loginRole;
      const dest = s.pending || (role === "leader" ? "explore" : "dash");
      patch({ user: role, pending: null });
      go(dest);
    },
